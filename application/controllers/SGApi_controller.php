	...
	{			 $this->load->helper('translation_helper');
				...
				if ($check) {				
			    	...
			    	if($this->input->post("current_lang")=="ar"){
 			        ...
 			        $result1 = $this->A->Offer_chk($date1,$vendor_id);
		    	    ...
		    	    $result = $this->A->whereFileNameSelected('vendor',$select,'id',$vendor_id,'status','1');	
		             ...
		             $result2 = $this->A->Offer_chk_user_history('claimed_offer_item',$select,'offer_type','membership','user_id',$user_id,'offer_id',$value->id);	
...
function Offer_details()
	{
				...
				if ($check) {				
			    	...
			    	$offer_id = $this->input->post("offer_id");
			     	$offer = $this->A->offer_detail_vendor($offer_id);	

					if($offer)
					{						
							$array13["status"]=true;
		  					$array13["message"]="success ";
		  					$array13["offer_detail"]=$offer;
					}else
					{		$array13["status"]=false;
		  					$array13["message"]="Nothing";
					}						
...
function Offer_RedeemByUser()
	{
				...
				if ($check) {				
			       ...
			       if(function_exists('date_default_timezone_set')) {
			    	  date_default_timezone_set("Asia/Kolkata");   }
					  $date1 =  date('Y-m-d H:i:s');
					  $voucher_code = mt_rand(10000, 99999);
				 $select = 'claimed_offer_id';
				 $select11 = '*';
			     $result = $this->A->where333FileNameSelected('claimed_offer_item',$select,'offer_type',$offer_type,'user_id',$user_id,'offer_id',$offer_id);	
			     	if (empty($result)) {
			     		 $userinfo = array(
				            'user_id'  => $user_id,
     					 ...
     					 $insert_id=$this->A->insert('claimed_offer_item',$userinfo);

			     	}else{
			     		$array_p =  array( 'voucher_code'  => $voucher_code,
			     							'claimed_status' => 1,
			     							'create_date' =>  $date1);
				        $insert_id = $this->A->updatewhere_order('claimed_offer_item','offer_type',$offer_type,'user_id',$user_id,'offer_id',$offer_id,$array_p);
			     	}
			     	 if ($insert_id) {
     					 	 $result11 = $this->A->where333FileNameSelected('claimed_offer_item',$select11,'offer_type',$offer_type,'user_id',$user_id,'offer_id',$offer_id);	
     					 }
...
function Scratch_offer_By_Branch_Code()
	{
		 		$this->load->helper('my_helper');
				$apikey = $this->input->get_request_header('token');
				$check=cofirmApiKey($apikey);
				if ($check) {				
			    	if($this->input->post("user_id") && $this->input->post("branch_code"))
			    	{
			    	$select = '*';
			    	$user_id = $this->input->post("user_id");
			    	$branch_code = $this->input->post("branch_code");
			        $saving_amount = $this->input->post("saving_amount");
			        $claimed_offer_id = $this->input->post("claimed_offer_id");
			        $offer_id = $this->input->post("offer_id");
			    	$final_update;
			    	$voucher_code;
			    	 if(function_exists('date_default_timezone_set')) {
			    	  date_default_timezone_set("Asia/Kolkata");   }
					  $date1 =  date('Y-m-d H:i:s');
			     	$result = $this->A->branch_code_Join($branch_code,$offer_id);
			     	
			     	if($result){
			     	
			     		 foreach ($result as $value2) {
			     		 	
			     			$final_update =	$this->A->updateUserOfferClaimed('claimed_offer_item',$user_id,$value2->offer_id,$value2->branch_id,$date1,$saving_amount,$claimed_offer_id);
			    		$voucher_code = $this->A->whereFileNameSelected('claimed_offer_item','voucher_code','offer_id',$value2->offer_id,'user_id',$user_id);

			    	if($final_update)
						{     
							$array13["status"]=true;
		  					$array13["message"]="success";
		  					$array13["voucher_code"]=$voucher_code;
					...
					{		$array13["status"]=false;
		  					$array13["message"]="Invalid Branch";
					}
					...
					$result11=$this->A->wheredetail_all_dec5('vendor_offers',$select,'category_id',$cat_id);	
							if ($j<5) {									    	    	   
			    	   		foreach ($result11 as $value) {
			    	   			@$result4[$i]->loc = $this->A->whereNearby("SELECT branch.id,branch.branch_name,branch.vendor_id,branch.latitude,branch.longitude, CAST((3959 *   acos(cos(radians('$lat_loc')) * cos(radians(latitude)) * cos(radians(longitude) - radians('$lng_loc')) +  sin(radians('$lat_loc')) * sin(radians(latitude )))) AS DECIMAL(10,1)) AS distance FROM old_selected_branch_offer LEFT JOIN branch ON branch.id=old_selected_branch_offer.branch_id WHERE old_selected_branch_offer.offer_id = '$value->id' HAVING  distance < 50 ORDER BY distance LIMIT 0,1");	
	...
	{			 $this->load->helper('translation_helper');
				...
				$check=cofirmApiKey($apikey);
				 if ($check) {
				   ...
				   $result11=$this->A->wherebyAllCat($cat_id);
			    	   
			   	foreach ($result11 as $value) {
			 	@$result[$i]->loc = $this->A->whereNearby("SELECT branch.id,branch.branch_name,branch.vendor_id,branch.latitude,branch.longitude, CAST((3959 *   acos(cos(radians('$lat_loc')) * cos(radians(latitude)) * cos(radians(longitude) - radians('$lng_loc')) +  sin(radians('$lat_loc')) * sin(radians(latitude )))) AS DECIMAL(10,1)) AS distance FROM old_selected_branch_offer LEFT JOIN branch ON branch.id=old_selected_branch_offer.branch_id WHERE old_selected_branch_offer.offer_id = '$value->id' HAVING  distance < 50 ORDER BY distance");		    	   		
			    	   			 $offer_title=translateText($value->offer_title);
					 	     $offer_detail=translateText($value->offer_detail);
					...
					if ($sub_cat == "no") {   							 // search only main category
			    	   	$result=$this->A->wherebyAllCat($cat_id);
			    	   		...
			    	   		foreach ($result as $value) {
			    	   			...
			    	   			@$result11[$i]->loc = $this->A->whereNearby("SELECT branch.id,branch.vendor_id,branch.latitude,branch.longitude,(3959 *   acos(cos(radians('$lat_loc')) * cos(radians(latitude)) * cos(radians(longitude) - radians('$lng_loc')) +  sin(radians('$lat_loc')) * sin(radians(latitude )))) AS distance FROM old_selected_branch_offer LEFT JOIN branch ON branch.id=old_selected_branch_offer.branch_id WHERE old_selected_branch_offer.offer_id = '$value->id' HAVING  distance < 50 ORDER BY distance ");
			    	   			 $offer_title=translateText($value->offer_title);
...
// ********************* Search offers ***********************************************
	...
	function Check_Saving()
	{
				...
				if ($check) {				
			    	...
			    	$select = '*';
			    	$user_id = $this->input->post("user_id");
			     	$result = $this->A->Saving_chk($user_id);

			     	$result11 = $this->A->total_sum_saving('claimed_offer_item','user_id',$user_id,'claimed_status','2');		 		    			    
					if($result11)
					{
							$array13["status"]=true;
		  					$array13["message"]="success ";
...
function Check_Points()
	{
				...
				if ($check) {				
			    	...
			    	$select = '*';
			    	$user_id = $this->input->post("user_id");
			     	$result = $this->A->Points_chk($user_id);
			    
			     	$result11 = $this->A->total_sum_Points('my_points','user_id',$user_id,'status',1);		 	 		    			    
					if($result11)
					{
							$array13["status"]=true;
		  					$array13["message"]="success ";
...
function Token_Insert()
	{
        	...
        	// per day login give some points to user 
					  ...
					  $date1 =  date('Y-m-d');    // point_setup_id = 2 mean "Daily Login"
                      $result = $this->A->where333FileNameSelected('my_points',$select,'point_setup_id',"2",'user_id',$user_id,'create_date',$date1);	
                   $reward_insert = "0";
                   $points = 0;
			     	if (empty($result)) {
			     		   $result12 = $this->A->whereFileNameSelected('points_setup','reward_points','status','Active','point_setup_id','2');
		            ...
		            $points =  $value->reward_points;
     					 ...
     					 $reward_insert=$this->A->insert('my_points',$userinfo);
     					}
						}
			 ...
			 // $other_points = $this->A->Points_chk_scratch('voucher_code_history',$select,'user_id',$user_id,'status',1,'read_sts',0);
					...
					if($insert_id)
					{
							$array13["status"]=true;
		  					$array13["message"]="success ";
		  					$array13["data"]=$insert_id;
		  					$array13["reward_sts"]=$reward_insert; // 0 for non 
		  					$array13["points"]=$points;     // defalut 0 points show 
...
function User_Subscription_Info()
	{
				...
				if ($check) {				
			    	...
			    	$user_id = $this->input->post("user_id");
			     	...
			     	if (empty($result)) {
			       	$result = $this->A->wheredetailtempLimit1('user_subscription_plan','status','Active');	
			       	foreach ($result as $value) {
			       	      $no_of_days = preg_replace("/[^0-9]/", "", $value->subscription_limit);
			       	    	$no_of_days1 = $no_of_days*30;
			       	      	 @$result[$i]->no_of_days =  $no_of_days1;
		             		          	  	
			        $i++;
			       	}
			       	@$result[0]->payment_status =  "No";	 		     
			    	}
					if($result)
					{
							$array13["status"]=true;
	...
	function Subscription_Plan_Insert()
	{
				...
				$check=cofirmApiKey($apikey);
				if ($check) {				
			    	if($this->input->post("user_id") && $this->input->post("transaction_no"))
			    	{
			    	  if(function_exists('date_default_timezone_set')) {
			    	  date_default_timezone_set("Asia/Kolkata");     }
                   ...
                   $no_of_days = $this->input->post("no_of_days");
                    $payment_status = $this->input->post("payment_sts");
                   ...
                   $uploadDate = date('Y-m-d');;
					...
					$date = date('Y-m-d', $date);
                        ...
                        $userinfo = array(				           
     					 ...
     					 $insert_id=$this->A->insert('transaction_all',$userinfo);
...
// ***********************   Level offer ******************
	...
	function Level_Offer_List()
	{
				...
				if ($check) {				
			    	...
			    	$select = '*';
			    	$user_id = $this->input->post("user_id");
			        $result1 = $this->A->wheredetail_all('level',$select,'status','Active');
			      	$result11 = $this->A->total_sum_Points('my_points','user_id',$user_id,'status',1);		 		    			    
			     	 $i = 0;  	
			          foreach ($result1 as $value) {
		             $result2 = $this->A->whereFileNameSelected('my_gift',$select,'user_id',$user_id,'level_id',$value->level_id);	
...
function Level_Offer_Redeem()
	{
				...
				if ($check) {				
			    	...
			    	$level_id = $this->input->post("level_id");
 					$voucher_code = mt_rand(10000, 99999);
			    	$insert_id = "";
			    	  $result1 = $this->A->wheredetail_senario_one('senario_gift',$select_senario,'level_id',$level_id);
			    	  foreach ($result1 as $value1) {

			    	   $result11 = $this->A->whereNearby("SELECT tbl.level_id, tbl.senario_number, tbl.senario_id,gift_id from ( SELECT gift_one_id as gift_id, level_id, senario_number, senario_id FROM senario_gift UNION SELECT gift_two_id as gift_id,level_id, senario_number, senario_id FROM senario_gift UNION SELECT gift_three_id as gift_id,level_id, senario_number, senario_id FROM senario_gift ) as tbl where tbl.level_id = $value1->level_id and senario_id = '$value1->senario_id' ORDER BY RAND() limit 1");
			    	    foreach ($result11 as $value2) {
			    	    $result12 = $this->A->wheredetail_gift_one('inventory_gift',$select,'id',$value2->gift_id);
			      if(function_exists('date_default_timezone_set')) {
			    	  date_default_timezone_set("Asia/Kolkata");     }
					  $date1 = date('Y-m-d');
		   			foreach ($result12 as $value) {
...
function Scratch_MyGift_By_Branch_Code()
	{
		 		$this->load->helper('my_helper');
				$apikey = $this->input->get_request_header('token');
				$check=cofirmApiKey($apikey);
				if ($check) {				
			    	...
			    	$user_id = $this->input->post("user_id");
			    	$branch_code = $this->input->post("branch_code");
			    	$claim_gift_id = $this->input->post("claim_gift_id");
			    	$vendor_id = $this->input->post("vendor_id");
			    	$final_update;
			    	$voucher_code;
			    	 if(function_exists('date_default_timezone_set')) {
			    	  date_default_timezone_set("Asia/Kolkata");   }
					  $date1 =  date('Y-m-d');
			     	$result = $this->A->whereFileNameSelected('branch','*','branch_code',$branch_code,'vendor_id',$vendor_id);
			     	
			     	if($result){			     	
			     		 foreach ($result as $value2) {
			     		 	$userinfo1 = array('claim_status'  => '2',
			     		 						'branch_id' =>  $value2->id,
			     		 						'update_date' => $date1);                   
                    $final_update=$this->A->updatewhere('my_gift','claim_gift_id',$claim_gift_id,$userinfo1);
			     		
			    		$voucher_code = $this->A->whereFileNameSelected('my_gift','voucher_code','claim_gift_id',$claim_gift_id,'claim_status','2');	
                  ...
                  if(function_exists('date_default_timezone_set')) {
			    	  date_default_timezone_set("Asia/Kolkata");   }
                        ...
                        $userinfo = array(
     					 ...
     					 $insert_id=$this->A->insert('users_activities',$userinfo);
