...
class Settings extends CI_Controller {
    ...
    public function country_save(){
        ...
        if(!empty($post)){
            ...
            $post = array_merge($blankArr, $post);
            $data = array(
                'country_name' => $post['Acountry_name_en'] ? $post['Acountry_name_en'] : translateTextEn($post['Acountry_name_ar']),
                'priority' => $post['Apriority'],
                'status'      => 'Active',
                'create_date' => date('yy-m-d')
            );

            if($_FILES['Acountry_flag']['name']!=""){
                $new_image_name = str_replace(str_split(' ()\\/,:*?"<>|'), '', 
                  $_FILES['Acountry_flag']['name']);
                ...
                $upload = $this->upload->do_upload('Acountry_flag');
