...
class Supports extends CI_Controller {
    ...
    public function rule_update(){
        ...
        if(!empty($post)){
            ...
            $data = array(
                'support_type' => 'rules',
                'title' => $post['Rtitle_en'] ? $post['Rtitle_en'] : translateTextEn($post['Rtitle_ar']),
                'description' => $post['Rdesc_en'] ? $post['Rdesc_en'] : translateTextEn($post['Rdesc_ar']),
                'priority' => $post['Rpriority'],
                'status'      => 'Active',
                'create_date' => date('yy-m-d')
            );
	...
	public function help_save(){
        $post = $this->input->post();
        if(!empty($post)){
            $this->form_validation->set_rules('HAtitle_en', 'Title En','required');
            $this->form_validation->set_rules('HAdesc_en', 'Description En','required');
            $this->form_validation->set_rules('HApriority', 'Priority','required');
            if ($this->form_validation->run() == FALSE) {
                ...
                $data = array(
                    'support_type' => 'help',
                    'title' => $post['HAtitle_en'] ? $post['HAtitle_en'] : translateTextEn($post['HAtitle_ar']),
                    'description' => $post['HAdesc_en'] ? $post['HAdesc_en'] : translateTextEn($post['HAdesc_ar']),
                    'priority' => $post['HApriority'],
                    'status'      => 'Active',
                    'create_date' => date('yy-m-d')
                );
    ...
    public function help_update(){
        ...
        if(!empty($post)){
            ...
            $data = array(
                'support_type' => 'help',
                'title' => $post['Htitle_en'] ? $post['Htitle_en'] : translateTextEn($post['Htitle_ar']),
                'description' => $post['Hdesc_en'] ? $post['Hdesc_en'] : translateTextEn($post['Hdesc_ar']),
                'priority' => $post['Hpriority'],
                'status'      => 'Active',
                'create_date' => date('yy-m-d')
            );
