<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	function get_vendorplan($vendor_id){
		if (empty($vendor_id)) {
			return false;
		}
		$CI = &get_instance();
		...
		$CI->db->select('*');
		$CI->db->from('transaction_all');
		$CI->db->where('user_id',$vendor_id);
		$CI->db->where('paid_by','vender');
		$CI->db->where('expire_account','No');
		$query = $CI->db->get();
		if($query->num_rows() > 0)	{
	...
	function region($sel = NULL, $country_id = NULL){
		...
		$option = '<option value="">Select Region</option>';
		foreach($resultArr as $val){
				$selVal = '';
				if(is_int($sel)){
					if($sel == $val['id']){
