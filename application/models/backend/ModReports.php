<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    ...
    class ModReports extends CI_Model{
        ...
        public function users_monthReport($data = NULL){
            $month = array('Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec');
            $output =array();
            if(empty($data['year'])){
                $data['year'] = date('yy');
            ...
            for ($i=0; $i < count($month); $i++) {
                $filter = $month[$i].'-'.$data['year'];
                $freeUser = getFreeUsers($filter);
                $ExpireUser = getExpireUsers($filter);
                $SubscribeUser = getSubscribeUsers($filter);
                $SubscribeUserValue = getSubscribeUserValue($filter);
                $offerRedeem = getuserofferRedeem($filter);
                $offerRedeemValue = getofferRedeemAmount($filter);
                $pointRedeem = getPointRedeem($filter);
                $giftRedeem = getGiftRedeem($filter);
                $giftValueRedeem = getGiftValueRedeem($filter);
                $StoreView = getUserStoreView($filter);
                $LinkShare = getLinkShare($filter);
                $RatedStore = getRatedStore($filter);
                $CommentStore = getCommentStore($filter);

                $output[] = array(
                    'month' => $filter,
                    'freeUser' => $freeUser['total_FreeUser'] ? $freeUser['total_FreeUser'] : 0,
                    'ExpireUser' => $ExpireUser['total_ExpireUser'] ? $ExpireUser['total_ExpireUser'] : 0,
                    'SubscribeUser' => $SubscribeUser['total_SubscribeUser'] ? $SubscribeUser['total_SubscribeUser'] : 0,
                    'SubscribeUserValue' => $SubscribeUserValue['total_UserValue'] ? $SubscribeUserValue['total_UserValue'] : 0,
                    'offerRedeem' => $offerRedeem['total_OfferRedeem'] ? $offerRedeem['total_OfferRedeem'] : 0,
                    'offerRedeemValue' => $offerRedeemValue['total_OfferRedeemValue'] ? $offerRedeemValue['total_OfferRedeemValue'] : 0,
                    'pointRedeem' => $pointRedeem['total_RedeemPoint'] ? $pointRedeem['total_RedeemPoint'] : 0,
                    'giftRedeem' => $giftRedeem['total_RedeemGift'] ? $giftRedeem['total_RedeemGift'] : 0,
                    'giftValueRedeem' => $giftValueRedeem['total_RedeemGiftValue'] ? $giftValueRedeem['total_RedeemGiftValue'] : 0,
                    'StoreView' => $StoreView['total_StoreView'] ? $StoreView['total_StoreView'] : 0,
                    'LinkShare' => $LinkShare['total_LinkShare'] ? $LinkShare['total_LinkShare'] : 0,
                    'RatedStore' => $RatedStore['total_RateStore'] ? $RatedStore['total_RateStore'] : 0,
                    'CommentStore' => $CommentStore['total_CommentStore'] ? $CommentStore['total_CommentStore'] : 0,
                );
            }
            return $output;
        }

        public function users_monthly_summary($data = NULL){
            $month = array('Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec');
            $output =array();
            if(empty($data['year'])){
                $data['year'] = date('yy');
            ...
            for ($i=0; $i < count($month); $i++) {
                $filter = $month[$i].'-'.$data['year'];
                $MonthUS = getMonthUSubscription($filter);
                $MonthVS = getMonthVSubscription($filter);
                $MonthC = getMonthClassifiedSale($filter);
                $MonthL = getMonthLimitedSale($filter);
                $MonthI = getMonthInventoryValue($filter);
                $MonthR = getMonthRedeemGift($filter);
                $MonthRG = getMonthRemainGift($filter);

                $output[] = array(
                    'month' => $filter,
                    'MonthUS' => $MonthUS['total_SubscribeUserValue'] ? $MonthUS['total_SubscribeUserValue'] : 0,
                    'MonthVS' => $MonthVS['total_SubscribeVenderValue'] ? $MonthVS['total_SubscribeVenderValue'] : 0,
                    'MonthC' => $MonthC['total_ClassifiedSale'] ? $MonthC['total_ClassifiedSale'] : 0,
                    'MonthL' => $MonthL['total_LimitedSale'] ? $MonthL['total_LimitedSale'] : 0,
                    'MonthI' => $MonthI['total_InventorySale'] ? $MonthI['total_InventorySale'] : 0,
                    'MonthR' => $MonthR['total_RedeemGift'] ? $MonthR['total_RedeemGift'] : 0,
                    'MonthRG' => $MonthRG['total_remainValue'] ? $MonthRG['total_remainValue'] : 0,
                );
            }
            return $output;
        }
