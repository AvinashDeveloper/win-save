            ...
            <div class="">
              ...
              <div class="table-responsive">
                <table class="table  display">
                  ...
                  <tbody>
                    <?php $i=1;
                      if(!empty($classifiedInfo)){
                        foreach($classifiedInfo as $value){
                    ?>
                    <tr>
                      <td><?php echo $i; ?></td>
                      <td><?php echo date('m/d/yy',strtotime($value['added_date'])); ?></td>
                      <td><?php echo $value['store_name']; ?></td>
                      <td><?php echo getCatgoryTypeString($value['category']); ?></td>
                      <td><?php echo $value['name']; ?></td>
                      <td><?php echo $value['title']; ?></td>
                      <td><button class="btn btn-success" onclick="getClassifiedItem('<?php echo $value['id']; ?>','<?php echo $value['vendor_id']; ?>');" data-toggle="modal" data-target="#updateClassifiedItem">Update</button></td>
...
<!-- The Modal -->
<div class="modal" id="updateClassifiedItem">
  <div class="modal-dialog">
    <div class="modal-content">
      <!-- Modal Header -->
      <div class="modal-header">
        <h4 class="modal-title">Update New Classified Approval</h4>
        <button type="button" class="close" data-dismiss="modal">&times;</button>
      </div>
      <!-- Modal body -->
      <div class="modal-body">
        <form action="" method="POST" id="UpdateClassifiedForm">
        <div class="row">
          ...
          <div class="row">
            <div class="col-6">
              <input type="hidden" name="classified_item_id" id="classified_item_id" value="">
              <input type="hidden" name="vender_id" id="vender_id" value="">
              <div class="form-group">
                <label for="">Date of Creation</label>
                <input type="date" name="create_date" id="create_date" class="form-control" placeholder="Date of Creation">
              </div>
              <div class="form-group">
                <label for="">Store Name</label>
                <input type="text" name="store_name" id="store_name" class="form-control" plcaholder="Store Name" readonly>
              </div>
              <div class="form-group">
            ...
            <div class="col-6">
              <div class="form-group">
                ...
                <input type="text" name="vender_name" id="vender_name"  class="form-control" plcaholder="Vendor Contact Creator" readonly>
              </div>
              <div class="form-group">
                <label for="">Title</label>
                <input type="text" name="classified_title" id="classified_title" class="form-control" plcaholder="classified title">
              </div>
            </div>
            <div class="col-12">
...
$("#warning").hide();
  ...
  var getClassifiedItem = function(classified_item_id, vender_id){
    $.ajax({
      type : "POST",
      url : "<?php echo base_url();?>backend/Item_verification/getClassifiedAprovalInfo",
      data : {classified_item_id:classified_item_id, vender_id:vender_id},
      success:function(data){
        var getInfo = JSON.parse(data);
        // console.log(getInfo.result);
        $("#vender_id").val(getInfo.result[0].vendor_id);
        $("#classified_item_id").val(getInfo.result[0].id);
        $("#create_date").val(getInfo.result[0].added_date);
        $("#store_name").val(getInfo.result[0].store_name);
        $("#category_id").val(getInfo.result[0].category);
        $("#vender_name").val(getInfo.result[0].name);
        $("#classified_title").val(getInfo.result[0].title);
      }
    })
  }
  ...
  //   var classified_item_id = $("#classified_item_id").val();
  //   var create_date = $("#create_date").val();
  //   var store_name = $("#store_name").val();
  //   var category_id = $("#category_id").val();
  //   var vender_name = $("#vender_name").val();
  //   var classified_title = $("#classified_title").val();

  //   $.ajax({
  //     type: 'POST',
  //     url: '<?php echo base_url();?>backend/Item_verification/classified_update',
  //     data : {vender_id:vender_id, classified_item_id:classified_item_id, create_date:create_date, store_name:store_name, category_id:category_id, vender_name:vender_name, classified_title:classified_title},
