...
public function whereFileNameSelected($tablename,$col,$col1,$id1,$col2,$id2)
 ...
 public function wherebyRatting($tablename,$col,$col1,$id1)
  ...
  public function wherebyRattingLimit($tablename,$col,$col1,$id1)
  {
    $this->db->select($col);
    $this->db->where($col1,$id1);
    $this->db->order_by('ratting', 'DESC');
  ...
  public function wherebyLatestDate($tablename,$col,$col1,$id1)
  {
   ...
   public function wherebyLatestDateLimit($tablename,$col,$col1,$id1)
  {
    $this->db->select($col);
    $this->db->where($col1,$id1);
    $this->db->order_by('add_date', 'DESC');
  ...
  public function wherebyPopular($tablename,$col,$col1,$id1)
  {
   ...
   public function wherebyPopularLimit($tablename,$col,$col1,$id1)
  {
    $this->db->select($col);
    $this->db->where($col1,$id1);
    $this->db->order_by('popular', 'DESC');
  ...
  public function whereNearby($lat11)
   ...
   public function whereLikeSubcatName($cat_id,$keyword)
      {      
        ...
        return $this->db->select('*')->from('offers')->where("`category_id` ='$cat_id'")->where("sub_cat_name LIKE '%$keyword%'")->get()->result_array();
      }
   public function whereLikeAmenitiesName($cat_id,$keyword)
      {      

        return $this->db->select('*')->from('offers')->where("`category_id` ='$cat_id'")->where("aminities LIKE '%$keyword%'")->get()->result_array();
      }
...
public function IncreassPopularitybyone($tablename,$col,$id){
 ...
 public function funLevel_Offer_join($offer_id)      //m.id as mainid
    {
    $this->db->select('v.*,m.name');
    $this->db->from('vendor_offers v');
    $this->db->join('vendor m', 'm.id = v.vendor_id', 'right');

    $this->db->where('v.id',$offer_id);
    $this->db->order_by('v.id','DESC');         
    $query = $this->db->get(); 
     return $query->result();
  }
