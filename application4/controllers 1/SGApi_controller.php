		            ...
		            foreach ($result1 as $value1){
					 	  $sub_cat_name=translateText($value1->sub_cat_name);
					 	   $aminities=translateText($value1->aminities);
					 	    $heading=translateText($value1->heading);
					 	     $message=translateText($value1->message);
 								@$result1[$j]->sub_cat_name =  $sub_cat_name;
 								@$result1[$j]->aminities =  $aminities;
 								@$result1[$j]->heading =  $heading;
 								@$result1[$j]->message =  $message;
 								$j++;
		             		 }
		         foreach ($result2 as $value1){
					 	  $sub_cat_name=translateText($value1->sub_cat_name);
					 	   $aminities=translateText($value1->aminities);
					 	    $heading=translateText($value1->heading);
					 	     $message=translateText($value1->message);
 								@$result2[$k]->sub_cat_name =  $sub_cat_name;
 								@$result2[$k]->aminities =  $aminities;
 								@$result2[$k]->heading =  $heading;
 								@$result2[$k]->message =  $message;
 								$k++;
		             		 }
		         foreach ($result3 as $value1){
					 	  $sub_cat_name=translateText($value1->sub_cat_name);
					 	   $aminities=translateText($value1->aminities);
					 	    $heading=translateText($value1->heading);
					 	     $message=translateText($value1->message);
 								@$result3[$l]->sub_cat_name =  $sub_cat_name;
 								@$result3[$l]->aminities =  $aminities;
 								@$result3[$l]->heading =  $heading;
 								@$result3[$l]->message =  $message;
 								$l++;
		             		 }
					}else{
			     	$result   = $this->A->wherebyFeaturedLimit('offers',$select,'status',1);
			     		...
			     		if (!empty($result_ratting)) {
			     		 @$result[0]->ratting_sts =  "1";	
							...
							$array13["status"]=1;
		  					...
		  					$array13["data"]=$result;
		  					$array13["data1"]=$result1;
		  					$array13["data2"]=$amenities;
		  					$array13["data4"]=$slider;
               ...
               foreach ($result as $value1){
					 	  $sub_cat_name=translateText($value1->sub_cat_name);
					 	   $aminities=translateText($value1->aminities);
					 	    $heading=translateText($value1->heading);
					 	     $message=translateText($value1->message);
 								@$result[$m]->sub_cat_name =  $sub_cat_name;
 								@$result[$m]->aminities =  $aminities;
 								@$result[$m]->heading =  $heading;
		            ...
		            foreach ($result1 as $value1){
					 	  $sub_cat_name=translateText($value1->sub_cat_name);
					 	   $aminities=translateText($value1->aminities);
					 	    $heading=translateText($value1->heading);
					 	     $message=translateText($value1->message);
 								@$result1[$n]->sub_cat_name =  $sub_cat_name;
 								@$result1[$n]->aminities =  $aminities;
 								@$result1[$n]->heading =  $heading;
 								@$result1[$n]->message =  $message;
 								$n++;
		             		 }
		         foreach ($result2 as $value1){
					 	  $sub_cat_name=translateText($value1->sub_cat_name);
					 	   $aminities=translateText($value1->aminities);
					 	    $heading=translateText($value1->heading);
					 	     $message=translateText($value1->message);
 								@$result2[$p]->sub_cat_name =  $sub_cat_name;
 								@$result2[$p]->aminities =  $aminities;
 								@$result2[$p]->heading =  $heading;
 								@$result2[$p]->message =  $message;
 								$p++;
		             		 }
		         foreach ($result3 as $value1){
					 	  $sub_cat_name=translateText($value1->sub_cat_name);
					...
					$result11=$this->A->wheredetail_all_dec5('offers',$select,'category_id',$cat_id);	
							if ($j<5) {									    	    	   
			    	   		foreach ($result11 as $value) {
			    	   			...
			    	   			@$result4[$i]->id = $value->id;
			    	   			@$result4[$i]->heading = $value->heading;
			    	   			@$result4[$i]->message = $value->message;
			    	   			@$result4[$i]->logo_img = $value->logo_img;	
			    	   			@$result4[$i]->featured_image = $value->featured_image;
			    	   			@$result4[$i]->discount = $value->discount;
			    	   			@$result4[$i]->add_date = $value->add_date;
	...
	{			 $this->load->helper('translation_helper');
				...
				$check=cofirmApiKey($apikey);
			    	...
			    	$result11 = array();
			    		...
			    		if($this->input->post("current_lang")=="ar"){
				          	...
				          	$result11=$this->A->wheredetail_all_dec5('offers',$select,'category_id',$cat_id);
			    	   		...
			    	   		foreach ($result11 as $value) {
			 	@$result[$i]->loc = $this->A->whereNearby("SELECT branch.id,branch.vendor_id,branch.latitude,branch.longitude, CAST((3959 *   acos(cos(radians('$lat_loc')) * cos(radians(latitude)) * cos(radians(longitude) - radians('$lng_loc')) +  sin(radians('$lat_loc')) * sin(radians(latitude )))) AS DECIMAL(10,1)) AS distance FROM selected_branch_offer LEFT JOIN branch ON branch.id=selected_branch_offer.branch_id WHERE selected_branch_offer.offer_id = '$value->id' HAVING  distance < 28 ORDER BY distance");		    	   		
			    	   			 $heading=translateText($value->heading);
					 	     $message=translateText($value->message);
			    	   			...
			    	   			@$result[$i]->discount = $value->discount;
			 	...
			 	@$result[$i]->loc = $this->A->whereNearby("SELECT branch.id,branch.vendor_id,branch.latitude,branch.longitude, CAST((3959 *   acos(cos(radians('$lat_loc')) * cos(radians(latitude)) * cos(radians(longitude) - radians('$lng_loc')) +  sin(radians('$lat_loc')) * sin(radians(latitude )))) AS DECIMAL(10,1)) AS distance FROM selected_branch_offer LEFT JOIN branch ON branch.id=selected_branch_offer.branch_id WHERE selected_branch_offer.offer_id = '$value->id' HAVING  distance < 28 ORDER BY distance");		    	   		
			    	   			...
			    	   			@$result[$i]->id = $value->id;
			    	   			@$result[$i]->heading = $value->heading;
	...
	{			 $this->load->helper('translation_helper');
				...
				$check=cofirmApiKey($apikey);
				 if ($check) {
			    	...
			    	$select = 'id,vendor_id,heading';
			    	$result11 = array();
			    		$i= 0;
			    if($this->input->post("current_lang")=="ar")
			    		{
					if ($sub_cat == "not") {   							 // search only main category
			    	   	$result=$this->A->wheredetail_all_dec5('offers',$select,'category_id',$cat_id);
			    	   
			    	   		foreach ($result as $value) {

			    	   			@$result11[$i]->loc = $this->A->whereNearby("SELECT branch.id,branch.vendor_id,branch.latitude,branch.longitude,(3959 *   acos(cos(radians('$lat_loc')) * cos(radians(latitude)) * cos(radians(longitude) - radians('$lng_loc')) +  sin(radians('$lat_loc')) * sin(radians(latitude )))) AS distance FROM selected_branch_offer LEFT JOIN branch ON branch.id=selected_branch_offer.branch_id WHERE selected_branch_offer.offer_id = '$value->id' HAVING (distance >='$distance_from' AND distance <= '$distance_to') ORDER BY distance ");
			    	   			 $heading=translateText($value->heading);
			    	   			@$result11[$i]->heading = $heading;
			    	   			$i++;
			    	   		}
			    	
			    	}else{
			    	$result=$this->A->wheredetail_all_dec5sub('offers',$select,'category_id',$cat_id,'sub_cat_name',$sub_cat);

			    	foreach ($result as $value) {
			    	   			@$result11[$i]->loc = $this->A->whereNearby("SELECT branch.id,branch.vendor_id,branch.latitude,branch.longitude,(3959 *   acos(cos(radians('$lat_loc')) * cos(radians(latitude)) * cos(radians(longitude) - radians('$lng_loc')) +  sin(radians('$lat_loc')) * sin(radians(latitude )))) AS distance FROM selected_branch_offer LEFT JOIN branch ON branch.id=selected_branch_offer.branch_id WHERE selected_branch_offer.offer_id = '$value->id' HAVING (distance >='$distance_from' AND distance <= '$distance_to') ORDER BY distance ");
			    	   			@$result11[$i]->heading = $value->heading;
			    	   			$i++;
			    	   		}
			    	}
			   	}else{
	...
	{				 $this->load->helper('translation_helper');
				...
				$check=cofirmApiKey($apikey);
				 if ($check) {
			    	...
			    	$select = '*'; 
			    	 if($this->input->post("current_lang")=="ar"){
			    	 $result = $this->A->whereFileNameSelected('offers',$select,'category_id',$cat_id,'status',1);
			      foreach ($result as $value1){
					 	  $sub_cat_name=translateText($value1->sub_cat_name);
					 	   $aminities=translateText($value1->aminities);
					 	    $heading=translateText($value1->heading);
				...
				$check=cofirmApiKey($apikey);
				 if ($check) {
			    	...
			    	$lng_loc = $this->input->post("lng");
 						...
 						$result1=$this->A->wheredetail_all_dec5('offers',$select,'category_id',$cat_id);
			    	...
			    	$result1=$this->A->wheredetail_all_dec5sub('offers',$select,'category_id',$cat_id,'sub_cat_name',$sub_cat_name);
			    	foreach ($result1 as $value) {
			    	   			@$result[$i]->loc = $this->A->whereNearby("SELECT branch.id,branch.vendor_id,branch.latitude,branch.longitude,CAST((3959 *   acos(cos(radians('$lat_loc')) * cos(radians(latitude)) * cos(radians(longitude) - radians('$lng_loc')) +  sin(radians('$lat_loc')) * sin(radians(latitude )))) AS DECIMAL(10,1)) AS distance FROM selected_branch_offer LEFT JOIN branch ON branch.id=selected_branch_offer.branch_id WHERE selected_branch_offer.offer_id = '$value->id' HAVING distance < 50 ORDER BY distance ");
			    	   			  $heading=translateText($value->heading);
			    	   			@$result[$i]->id = $value->id;
				...
				$check=cofirmApiKey($apikey);
				 if ($check) {
					  ...
					  $date1 =  date('Y-m-d H:i:s');
				 		...
				 		if ($type == '1') {
				 			$cat_name = $this->input->post("cat_name");
				 		$result1 = $this->A->whereNearby("SELECT * FROM limited_offer WHERE `category_name` ='$cat_name' && `valid_date` >= '$date1'  LIMIT 30 ");			 			
				 		}else if ($type == '2') {
				 			$cat_name = $this->input->post("cat_name");
				 		    $region = $this->input->post("region");

				 		$result1 = $this->A->whereNearby("SELECT * FROM limited_offer WHERE `category_name` ='$cat_name' && `region` = '$region' && `valid_date` >= '$date1' ");	
				 			 
				 		}else{
				 		    $cat_name = $this->input->post("cat_name");
				 		    $region = $this->input->post("region");
				 		    $section = $this->input->post("section");
				 			$result1 = $this->A->whereNearby("SELECT * FROM limited_offer WHERE `category_name` ='$cat_name' && `region` = '$region' && `section` = '$section'  && `valid_date` >= '$date1' ");	
				 		} 		
...
function AboutUs_Info()
	{
				...
				if ($check) {				
			    	...
			    	$select = '*';
			    	$user_id = $this->input->post("user_id");
			        $result1 = $this->A->whereFileName('about_us',$select);
	...
	function Contact_Info()
	{
				...
				if ($check) {				
			    	...
			    	$select = '*';
			    	$user_id = $this->input->post("user_id");
			        $result1 = $this->A->whereFileName('contact_us',$select);
...
function Rules_List()
	{
				...
				if ($check) {				
			    	...
			    	$select = '*';
			    	$user_id = $this->input->post("user_id");
			        $result1 = $this->A->whereFileName('rules',$select);
	...
	function Help_List()
	{
				...
				if ($check) {				
			    	...
			    	$user_id = $this->input->post("user_id");
			        $result1 = $this->A->whereFileName('help_que_ans',$select);
